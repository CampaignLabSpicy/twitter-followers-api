const debug = require('debug')('kyf:location:helpers')
...
const partsFromPostcode = pc => {
  ...
  if (match7) { return [match7[1], match7[2]] }
  if (match8) { return [match8[1], match8[2]] }
  if (isPostcodeDistrict(pc)) { return [pc.match(pcdRegexp)[1], null] }
}
...
const toLatLong = latLong => {
  ...
  if (Array.isArray(latLong) && latLong.length === 2 && typeof (latLong[0]) === 'number' && typeof (latLong[1]) === 'number') { return ({ lat: latLong[0], lng: latLong[0] }) }
  if (typeof latLong === 'object') {
    return (latLong.lat === undefined || latLong.lng === undefined)
      ? null
      : latLong
  }
  if (typeof latLong === 'string') { debug('##', latLong) }
  debug('###', latLongFromString(latLong))
  if (typeof latLong === 'string') { return latLongFromString(latLong) }
  return null
}

const toStandardLatLong = latLong => {
  const result = toLatLong(latLong)
  if (!result || result.lat === undefined || result.lng === undefined) { return null }
  debug(result, { lat: roundToNearest(0.001)(result.lat), lng: roundToNearest(0.001)(result.lng) })
  return {
...
const latLongFromString = latLong => {
  ...
  if (eastWest) { latLong = `${eastWest[1]}${eastWest[3]}${eastWest[5]}` }
  const matchDigits = latLong.match(latLongDigitsRegExp)
  if (!matchDigits) { return null }
  let [_, lng, lat] = matchDigits
  debug('Found something:', lat, lng)
  lat = 1 * lat
  lng = 1 * lng
  if (eastWest) {
    if (eastWest[2] === 'S' || eastWest[2] === 's') { lat = -lat }
    if (eastWest[4] === 'W' || eastWest[4] === 'w') { lng = -lng }
  }
  return { lat, lng }
}

const latLongFrom4dpLatLongString = latLong => {
  if (!latLong || typeof latLong !== 'string') { return null }
...
const withAt = handle => {
  const resultGroup = handle.match(leadingWhitespaceAtWhitespaceRegExp)
  return resultGroup
    ? `@${resultGroup[1]}`
    : handle
}
...
module.exports = {
  ...
  pc7FromFullPostcode,
  pc8FromFullPostcode,
  districtFromFullPostcode,
  districtFromPostcodeDistrict,
  postcodeFromString,
  toStandardLatLong,
  toLatLong,
  isLeafletLatLng,
  latLongFromString,
  latLongFrom4dpLatLongString,
  roundToNearest,
  latLongIsInBritishIsles,
  promiseyLog,
  withAt
}
